...
/**
 * @param loads defaults to `input.loads`. 220.83 passes the merged existing +
 * new list, because an addition's lighting is general lighting too — reading
 * only `input.loads` there silently valued it at zero.
 */
function lightingVA(input: LoadCalcInput, loads: LoadItem[] = input.loads): number {
  const extra = loads
...
export function existingDwellingAmps(
  ...
  const lighting = lightingVA(input, all)
  const fixedCircuitVA = (counts.smallAppliance + counts.laundry) * 1500
  ...
  const applianceVA = appliances.reduce((s, l) => s + resolveVA(l), 0)
  const otherVA = lighting + fixedCircuitVA + applianceVA

  // The breakdown is built here rather than in a wrapper so the number and its
  // own explanation can never drift apart.
  const breakdown: BreakdownLine[] = [
    {
      label: `General lighting — ${input.floorAreaSqFt} ft² @ 3 VA/ft²`,
    ...
    {
      label: `${counts.smallAppliance} small-appliance + ${counts.laundry} laundry circuit(s) @ 1500 VA`,
      type: 'smallAppliance',
      rawVA: fixedCircuitVA,
      appliedVA: fixedCircuitVA,
      rule: '220.83 / 220.52',
    },
    ...appliances.map((item) => ({
      label: item.label,
      type: item.type,
      rawVA: resolveVA(item),
      appliedVA: resolveVA(item),
      rule: '220.83 nameplate',
    })),
  ]
  ...
  if (addingHvac) {
    ...
    breakdown.push({
      label: `${hvacAll.winner === 'heat' ? 'Space heating' : 'Air conditioning'} governs (heat ${hvacAll.heatVA} VA vs A/C ${hvacAll.coolVA} VA)`,
      type: hvacAll.winner === 'heat' ? 'spaceHeat' : 'cooling',
      rawVA: hvacAll.winnerVA,
      appliedVA: hvacAll.winnerVA,
      rule: '220.83(B) — larger of the two at 100%, noncoincident',
    })
    breakdown.push({
      label: 'All other load — first 8 kVA @ 100%, remainder @ 40%',
      type: 'other',
      rawVA: otherVA,
      appliedVA: Math.min(otherVA, 8000) + 0.4 * Math.max(0, otherVA - 8000),
      rule: '220.83(B)',
    })
  } else {
    variant = 'A'
    ...
    if (hvacAll.winnerVA > 0) {
      breakdown.push({
        label: `Existing ${hvacAll.winner === 'heat' ? 'space heating' : 'air conditioning'} (staged with all other load)`,
        type: hvacAll.winner === 'heat' ? 'spaceHeat' : 'cooling',
        rawVA: hvacAll.winnerVA,
        appliedVA: hvacAll.winnerVA,
        rule: '220.83(A)',
    ...
    breakdown.push({
      label: 'Total connected load — first 8 kVA @ 100%, remainder @ 40%',
      type: 'other',
      rawVA: pile,
      appliedVA: totalVA,
      rule: '220.83(A)',
  ...
  // A calculation built on guessed nameplates is a different quality of document
  // from one built on plate readings, and the report says which it is.
  const assumedValues = all
    .filter((l) => !CODE_FIXED_TYPES.has(l.type) && l.nameplateRead !== true)
    .map((l) => l.label)

  const amps = roundAmps(totalVA / input.serviceVoltage)
  return { variant, totalVA, amps, fits: amps <= input.serviceAmps, breakdown, assumedValues }
}

/**
 * 220.83 as the field decision it actually is: can this existing service take
 * the load the customer wants to add?
 *
 * This is the first-pass check on every "add load to an existing dwelling" call
 * — an EV charger, a heat pump, a hot tub. It is fast, defensible and
 * code-cited, and it needs no return trip. Run it first: if it clears, quote the
 * addition; if it doesn't, quote the service upgrade. 220.87's metered-demand
 * study is the exception, not the fallback (see `existingDemandAmps`).
 *
 * Pure and deterministic — no ids, no timestamps, no I/O. Those are the
 * persistence layer's job, and loadcalc.test.ts runs the engine fifty times and
 * compares the results to prove it.
 */
export interface CapacityCheck220_83 {
  method: '220.83'
  variant: 'A' | 'B'
  totalVA: number
  amps: number
...
export function capacityCheck220_83(
  input: LoadCalcInput,
  newLoads: LoadItem[] = [],
): CapacityCheck220_83 {
  // Calls existingDwellingAmps rather than reimplementing 220.83 — two copies of
  // a code calculation is two calculations, and one of them will be wrong.
  const result = existingDwellingAmps(input, newLoads)
  return {
    method: '220.83',
    variant: result.variant,
    totalVA: result.totalVA,
    amps: result.amps,
    serviceAmps: input.serviceAmps,
    loadPct: Math.round((result.amps / input.serviceAmps) * 1000) / 10,
    spareAmps: input.serviceAmps - result.amps,
    fits: result.fits,
    breakdown: result.breakdown,
    assumedValues: result.assumedValues,
    citation:
      result.variant === 'B'
        ? '220.83(B) — new air conditioning or space heating: larger of the two at 100%, plus the first 8 kVA of all other load at 100% and the remainder at 40%.'
        : '220.83(A) — no new air conditioning or space heating: first 8 kVA of the total connected load at 100%, remainder at 40%.',
  }
}

// ── 220.87 — existing loads by measured maximum demand ──────────────────────
//
// The fallback, not the default. It exists for the case where 220.83 says the
// service is too small and the customer would rather pay for a study than an
// upgrade. It costs a recorder, a return trip and thirty days, and if the study
// comes back short the upgrade is needed anyway and the fee is spent.
//
// It also measures the house *as it was used*. A rental about to gain three
// occupants can pass on last winter's data and still be the wrong basis for
// sizing a service. Say so out loud, or this quietly becomes the path that talks
// customers out of upgrades they need.

export type DemandDataSource = 'utility_12mo' | 'recorded_30day'

export interface DemandRecord {
  source: DemandDataSource
  /** Highest 15-minute demand observed, in VA. */
  measuredMaxDemandVA: number
  /** Consecutive months of utility data. 220.87(1) requires 12. */
  monthsOfData?: number
  /** Length of the recording, in days. 220.87(2) requires 30. */
  recordedDays?: number
  /** Demand interval in minutes. 220.87 specifies 15. */
  intervalMinutes?: number
  windowStart?: string
  windowEnd?: string
}

export interface DemandCheck220_87 {
  method: '220.87'
  /** False when the data does not meet 220.87's own requirements. */
  qualifies: boolean
  /** Objective code failures. Present only when `qualifies` is false. */
  disqualifications: string[]
  /** Judgement calls the reader has to weigh. Stated, never used to block. */
  limitations: string[]
...
/**
 * Existing load = 125% of the measured maximum demand, plus the new load at
 * nameplate.
 *
 * The 125% attaches to the *measured demand* — 220.87 says so. The new load
 * enters at nameplate here; its own continuous factor belongs to the 230.42
 * check, which `futureFindings()` already applies.
 *
 * Pure: `windowStart`/`windowEnd` are read as strings and never compared against
 * the clock, so the same record always yields the same answer.
 */
export function existingDemandAmps(
  record: DemandRecord,
  newLoads: LoadItem[],
  serviceAmps: number,
  ...
  // These are objective requirements in the code, so they hard-block rather than
  // produce a number nobody could defend.
  if (record.source === 'utility_12mo') {
    const months = record.monthsOfData ?? 0
    if (months < 12) {
      disqualifications.push(
        `220.87(1) requires 12 consecutive months of utility demand data; ${months} month(s) supplied.`,
      )
    }
  } else {
    const days = record.recordedDays ?? 0
    if (days < 30) {
      disqualifications.push(
        `220.87(2) requires a minimum 30-day recording where 12 months of data is unavailable; ${days} day(s) supplied.`,
      )
    }
  }
  const interval = record.intervalMinutes ?? 15
  if (interval !== 15) {
    disqualifications.push(
      `220.87 specifies a 15-minute demand interval; this recording used ${interval} minutes.`,
    )
  }
  if (!(record.measuredMaxDemandVA > 0)) {
    disqualifications.push('No maximum demand was recorded.')
  }

  // Judgement, not code. Stated so the reader can weigh them — blocking on these
  // would be the engine substituting its opinion for an electrician's.
  const limitations: string[] = [
    'This reflects how the property was actually used during the recorded window, under its occupancy at the time. A change in occupants or usage changes the answer.',
  ]
  if (record.windowStart && record.windowEnd) {
    limitations.push(
      `Recorded ${record.windowStart} – ${record.windowEnd}. Confirm the window covers this property's peak season before relying on it.`,
    )
  }

  const existingDemandVA = 1.25 * (record.measuredMaxDemandVA || 0)
  const newLoadVA = newLoads.reduce((sum, item) => sum + resolveVA(item), 0)
  const totalVA = existingDemandVA + newLoadVA
  const amps = roundAmps(totalVA / serviceVoltage)

  return {
    method: '220.87',
    qualifies: disqualifications.length === 0,
    disqualifications,
    limitations,
    existingDemandVA,
    newLoadVA,
    ...
    amps,
    serviceAmps,
    loadPct: Math.round((amps / serviceAmps) * 1000) / 10,
    spareAmps: serviceAmps - amps,
    fits: amps <= serviceAmps,
    citation:
      '220.87 — existing load taken as 125% of the maximum demand recorded over 12 months (or a 30-day recording at 15-minute intervals), plus the new load.',
  }
}
...
export function calculateLoad(input: LoadCalcInput): LoadCalcResult {
  ...
  if (methodUsed === 'optional') {
    ...
    // Deliberately NOT passing input.futureLoads: futureFindings() below adds
    // each of them on top of governingAmps, so folding them in here would count
    // the same load twice. "Can I add this?" is capacityCheck220_83's question,
    // and it answers it without double-counting.
